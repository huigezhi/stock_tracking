...
| 全市场扫描 | webui.py `_div_scanner` | 每交易日16:00后全A扫描，结果存 div_hist.json |
...
**涉及文件**：webui.py（Handler.do_GET/do_POST/do_DELETE 入口处）、static/app.js（fetch 封装统一注入头 + 401 跳登录）、config.example.json
...
**涉及文件**：webui.py（signal_track 读写 + 回填线程 + `/api/stats` 聚合接口）、static/app.js + index.html（复盘面板）、新增 backtest.py（二期）
...
**背景**：`div_hist.json`/`kline_cache.json`/`etf_share_hist.json`/`state.json` 全量读写，无事务；kline_cache 单文件已 44KB，全A自选扩展后膨胀；JSON 无法做时间范围查询，制约 P0-3。
...
**风险**：迁移代码路径多，建议逐表分 PR；kline_cache 迁移后 `.gitignore` 改为 `data.db`。
...
**详细设计**：
- `manifest.json`（name/icons/theme_color/standalone）
...
**背景**：问财证明了"一句话选股"的需求；本项目已有全市场数据底座（all_stocks.json + K线缓存 + 背离扫描），接 LLM 可低成本复刻并超越（因为指标口径自控）。
...
- 份额异动提醒：现有 etf_share_hist 数据上做环比突变检测，大额申赎推送飞书（机构动向代理指标）
