...
- **中栏 · 底背离标的面板**：K线图下方表格
  - **全部A股**（约5200只，不含北交所）日线 / 周线底背离全量扫描，每日自动重扫一次（约6分钟，含实时进度显示）
  - 只保留**最近100个周期内**成立的底背离，更早周期自动忽略
  - **30天滚动缓存**（`div_hist.json`）：扫描结果按日持久化，仅保留最近30个扫描日，第31天自动淘汰第1天；当日已扫过（含服务重启）直接读缓存不重扫，大幅降低资源消耗
  - 表格上方**筛选栏**：按扫描日期 / 周期（日线、周线）/ 名称代码关键字组合筛选，实时显示命中条数
  - 展示序号、股票基本信息（名称 / 代码）、底背离日期区间、价格与 DIF 变化、确认日期、发现日期，按确认日期倒序
